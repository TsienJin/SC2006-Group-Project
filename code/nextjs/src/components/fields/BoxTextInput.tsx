...
const BoxTextInput = ({placeholder, defaultVal, required=false, type="text", forceErrorMessage="", validateTests=[], hoist=(value)=>{}}:{placeholder:string, defaultVal?:string|number|any, required?:boolean, type?:"text"|"password"|"textarea", forceErrorMessage?:string, validateTests?:ValidateInputText[], hoist?:Hoist<string>}) => {
    ...
    <FieldWrapper>
      <div className="relative">
        <div className="relative flex flex-row justify-center items-center">
          <label className={`
                transition-all text-shadow first-letter:uppercase pr-2
                ${valid()?"":"text-rust"}
          `} htmlFor={placeholder}>{placeholder}</label>
          <input className={inputClassName} onChange={e=>validate(e)} onBlur={()=>usrClicked()} defaultValue={defaultVal} type={type} name={placeholder} placeholder=" "/>
          <span className={`transition absolute right-1 top-2 text-rust ${valid()?"opacity-0 display-none":"opacity-100"}`}>
